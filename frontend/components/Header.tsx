  ...
  return (
    ...
    >
      <nav className="container mx-auto px-4 lg:px-8">
        ...
        >
          <div className="py-8 px-4 bg-white dark:bg-gray-800 rounded-b-[2rem] shadow-2xl border-t-4 border-[#A97E50]">
            {navLinks.map((link, index) => (
              <Link
                key={link.name}
                href={link.href}
                onClick={() => setIsMobileMenuOpen(false)}
                className="block px-6 py-4 mb-3 text-xl font-bold text-gray-800 dark:text-white bg-gray-50 dark:bg-gray-700 hover:bg-gradient-to-r hover:from-[#A97E50] hover:to-[#C4A86D] hover:text-white rounded-[1.5rem] transition-all duration-300 transform hover:scale-110 hover:-rotate-1 shadow-lg hover:shadow-2xl animate-bounceIn"
                style={{ animationDelay: `${index * 150}ms` }}
              >
                <span className="flex items-center space-x-4">
                  <span className="w-3 h-3 bg-gradient-to-r from-[#A97E50] to-[#C4A86D] rounded-full animate-pulse"></span>
                  <span>{link.name}</span>
                </span>
              </Link>
            ))}
            <div className="mt-6 pt-6 border-t-4 border-dashed border-[#C4A86D]">
              <button
                onClick={() => setIsDarkMode(!isDarkMode)}
                className="flex items-center justify-between w-full py-5 px-6 text-xl font-bold bg-gradient-to-r from-[#A97E50] to-[#C4A86D] text-white rounded-[1.5rem] transition-all duration-300 transform hover:scale-110 hover:rotate-2 shadow-lg hover:shadow-2xl animate-bounceIn"
                style={{ animationDelay: `${navLinks.length * 150}ms` }}
              >
                <span className="flex items-center space-x-4">
                  {isDarkMode ? (
                    <>
                      <svg className="w-8 h-8 animate-spin" style={{ animationDuration: '8s' }} fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M10 2a1 1 0 011 1v1a1 1 0 11-2 0V3a1 1 0 011-1zm4 8a4 4 0 11-8 0 4 4 0 018 0zm-.464 4.95l.707.707a1 1 0 001.414-1.414l-.707-.707a1 1 0 00-1.414 1.414zm2.12-10.607a1 1 0 010 1.414l-.706.707a1 1 0 11-1.414-1.414l.707-.707a1 1 0 011.414 0zM17 11a1 1 0 100-2h-1a1 1 0 100 2h1zm-7 4a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zM5.05 6.464A1 1 0 106.465 5.05l-.708-.707a1 1 0 00-1.414 1.414l.707.707zm1.414 8.486l-.707.707a1 1 0 01-1.414-1.414l.707-.707a1 1 0 011.414 1.414zM4 11a1 1 0 100-2H3a1 1 0 000 2h1z" clipRule="evenodd" />
                      </svg>
                    ...
                    <>
                      <svg className="w-8 h-8 animate-spin" style={{ animationDuration: '20s' }} fill="currentColor" viewBox="0 0 20 20">
                        <path d="M17.293 13.293A8 8 0 016.707 2.707a8.001 8.001 0 1010.586 10.586z" />
                      </svg>
                      <span>Dark Mode</span>
                    </>
